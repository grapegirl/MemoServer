...
<body>
	...
	<%
		...
		ArrayList<IP> list = ServerDao.selectIP("SELECT * FROM IP WHERE version_code = " + idx + ";");

		if (list.size() == 1) {
			System.out.println("글 수정 데이타 조회");
			System.out.println(list.get(0).getVersionCode());
			System.out.println(list.get(0).getVersionName());
		} else {
		...
		<form name=board method=post>
			...
			<tr height="10">
				...
				<td><input name="versionCode" type=edit
					value="<%=list.get(0).getVersionCode()%>" maxlength="100">
					<input name="idx" type="hidden" value=<%=idx%>></td>
			</tr>
			<tr height="10">
				<td width=70>버전명</td>
				<td><input name="versionName" type=edit
					value="<%=list.get(0).getVersionName()%>" maxlength="100">
				</td>
			</tr>
		<tr height="10">
			<td width="10"></td>
